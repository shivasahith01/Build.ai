...
export function AIChatbot() {
  ...
  const [context, setContext] = useState<{ lastQuery: string; lastResponse: any } | null>(null);

  const translateQuery = async (input: string) => {
    const response = await fetch('https://api.deepl.com/v2/translate', {
      method: 'POST',
      headers: { 'Authorization': 'DeepL-Auth-Key YOUR_API_KEY', 'Content-Type': 'application/json' },
      body: JSON.stringify({ text: [input], target_lang: 'EN' }),
    });
  ...
  const handleQuerySubmit = async (input: string): Promise<void> => {
    ...
    const translatedInput = language !== 'en-US' ? await translateQuery(input) : input;
    console.log(translatedInput);
    console.log(input);

    if (context && input.includes('filter')) {
      finalQuery = `${context.lastQuery} ${input}`;
    }
    ...
    const apiUrl = findBestMatch(translatedInput);
    console.log(apiUrl);
    try {
      if (apiUrl) {
        ...
        const response = await fetch(`${config.api_base_url}/chat`, {
          ...
          body: JSON.stringify({ query: translatedInput }),
        });
        ...
        const processStream = async () => {
          ...
          setContext({ lastQuery: finalQuery, lastResponse:botMessage});
